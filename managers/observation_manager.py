...
class ObservationManager(ManagerBase):
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        for group_name, group_term_dims in self._group_obs_term_dim.items():
            ...
            self._group_obs_dim[group_name] = (49,)
    ...
    def compute_group(self, group_name: str) -> torch.Tensor | dict[str, torch.Tensor] | list:
        ...
        if self._group_obs_concatenate[group_name]:
            # print(group_obs)
            # print("+++++++++++++++++++++++++++++++++++++++")
            # print(group_obs.values())
            # input()
            return torch.cat(list((group_obs["joint_pos"], group_obs["joint_vel"], group_obs["actions"])), dim=-1), torch.cat((group_obs["head_camera"], group_obs["wrist_camera"]), dim = -1)
            # return list(group_obs.values())
        else:
            return group_obs

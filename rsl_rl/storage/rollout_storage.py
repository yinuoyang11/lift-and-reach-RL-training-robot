...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, actions_shape, device="cpu"):
        ...
        self.observations = torch.zeros(num_transitions_per_env, num_envs, obs_shape[0][0], obs_shape[0][1], obs_shape[0][2], device=self.device)
        if privileged_obs_shape[0] is not None:
            self.privileged_observations = torch.zeros(
                num_transitions_per_env, num_envs, privileged_obs_shape[0][0], privileged_obs_shape[0][1], privileged_obs_shape[0][2], device=self.device
            )
        else:
